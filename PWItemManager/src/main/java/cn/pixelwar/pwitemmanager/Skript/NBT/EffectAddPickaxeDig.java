...
public class EffectAddPickaxeDig extends Effect {
    private Expression<Player> playerin;
    private Expression<Number> xpin;
    private Expression<ItemType> itemin;

    static {
        Skript.registerEffect(EffectAddPickaxeDig.class, new String[]{
                "addpickaxedig %player% %number% %itemtypes%"
        });
    }
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "update dig num of item: " + playerin.toString(event, debug);
    }
    ...
    protected void execute(Event e) {
        ...
        int num = xpin.getSingle(e).intValue();
        ItemStack item = itemin.getSingle(e).getRandom();
        updateSlotLore.addPickaxeDig(player, num, item);
    }
